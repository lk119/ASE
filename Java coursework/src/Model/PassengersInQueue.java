...
//		         for (Message mesg : getMessages()) {
//						
//						System.out.println(mesg.getContents());
//		         }
...
//		         System.out.println(message);
	...
	public Passenger BusinessFirstPicker() {
		...
		try {
			...
			while (true) {
				...
				if (window.getpClass().equalsIgnoreCase("Business") || window.getpClass().equalsIgnoreCase("First") ) {
					p = q.take();
	...
	public void securityQueue(Passenger p) {
		...
		try {
			...
			//put customer p into the security queue
			security.put(p);
...
//This method will put customers in a priority boarding queue following security
	...
	public Passenger PickerforBoarding() {
		...
		try {
			...
			if (window.getCheckInStatus() == true) {
				...
				// this can be used for GUI output- may be redundant....
				getBoardingReport();
	...
	public String getBoardingReport() {
		
		String boardingreport = null;
		Passenger p= null;
		try {
			p = security.take();
			boardingreport =p.getpClass() + "REPORT";
		...
		System.out.println(boardingreport);
