...
public class GUIMain extends JFrame implements Observer {
	...
	private JPanel northPanel, centerPanel, southPanel, FirstandBusinessDesk, economyDesk1, economyDesk2, economyDesk3,processButtonPanel,
			...
			baVolumeProgresBarPanel, baWeightProgresBarPanel, qaProgressBarPanel,
			qaVolumeProgresBarPanel, qaWeightProgresBarPanel, eaProgressBarPanel, eaVolumeProgresBarPanel, eaWeightProgresBarPanel;
	...
	private JLabel desk1Lbl, ecoDesk1Label, ecoDesk2Label, ecoDesk3Label, checkinQueuelabel, firstcheckinQueuelabel,
			...
			baWeightLabel, qaVolumeLabel,qaWeightLabel, eaVolumeLabel,eaWeightLabel ;
	...
	private JProgressBar baWeight, baVolume, qaWeight, qaVolume, eaWeight, eaVolume;
...
//		FirstandBusinessDesk.add(buttonPanel1, BorderLayout.SOUTH);
	...
	private void setupSouthPanel() {
		...
		eaProgressBarPanel.setLayout(new BorderLayout(0,0));
		...
		baProgressBarPanel.setLayout(new BorderLayout(0,0));
		...
		baProgressBarPanel.add(baVolumeProgresBarPanel, BorderLayout.CENTER);
		...
		baProgressBarPanel.add(baWeightProgresBarPanel, BorderLayout.SOUTH);
		...
		qaProgressBarPanel.setLayout(new BorderLayout(0,0));
	...
	public void updateFlightVolumes(String carrier) {
		switch(carrier) 
		{
	...
	public void disableProcessButton() {
		...
		SwingWorker<Passenger, String> EconomyWorker = new SwingWorker<Passenger, String>() {
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
						...
						passengersinqueue.economyQueueputi();

						// iterates through the link list of passengers in the queue in real time and
						// adds them to a string
						Iterator<Passenger> iteratoreco = passengersinqueue.economyQueueputi().iterator();
						String geco = "";
						while (iteratoreco.hasNext()) {
							geco = geco + (iteratoreco.next().getFullName()) + "\n";

						}
						publish(geco);
					} catch (NullPointerException | InterruptedException N) {
						//System.out.println("The economy class check-in queue is empty");
					}
			...
			protected void process(List<String> chunks) {
				String geco = chunks.get(chunks.size() - 1);
				checkinQueueTxtArea.setText(geco);
			...
			protected void done() {
				try {
					...
					checkinQueueTxtArea.setText("Done");
		...
		SwingWorker<Passenger, String> FirstWorker = new SwingWorker<Passenger, String>() {
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
						...
						passengersinqueue.firstbusinessQueueputi();

						// iterates through the link list of passengers in the queue in real time and
						// adds them to a string
						Iterator<Passenger> iteratorfirst = passengersinqueue.firstbusinessQueueputi().iterator();
						String gfirst = "";
						while (iteratorfirst.hasNext()) {
							gfirst = gfirst + (iteratorfirst.next().getFullName()) + "\n";
			...
			@Override
			protected void process(List<String> chunks) {
				String gfirst = chunks.get(chunks.size() - 1);
				firstcheckinQueueTxtArea.setText(gfirst);
			}
		};

		SwingWorker<Passenger, String> SecurityWorker = new SwingWorker<Passenger, String>() {
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
						...
						passengersinqueue.securityPicker();

						// iterates through the link list of passengers in the queue in real time and
						// adds them to a string
						Iterator<Passenger> iteratorsecurity = passengersinqueue.securityPicker().iterator();
						String gsecurity = "";
						while (iteratorsecurity.hasNext()) {
							gsecurity = gsecurity + (iteratorsecurity.next().getFullName()) + "\n";
			...
			@Override
			protected void process(List<String> chunks) {
				String gsecurity = chunks.get(chunks.size() - 1);
				securityQueueTxtArea.setText(gsecurity);
			}
		};

		SwingWorker<Passenger, String> BoardingWorker = new SwingWorker<Passenger, String>() {
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
						...
						passengersinqueue.boardingPicker();

						// iterates through the link list of passengers in the queue in real time and
						// adds them to a string
						Iterator<Passenger> iteratorboarding = passengersinqueue.boardingPicker().iterator();
						String gboarding = "";
						while (iteratorboarding.hasNext()) {
							gboarding = gboarding + (iteratorboarding.next().getFullName()) + "\n";
						}
						publish(gboarding);
			...
			@Override
			protected void process(List<String> chunks) {
				String gboarding = chunks.get(chunks.size() - 1);
				boardingQueueTxtArea.setText(gboarding);
			}
		};

		
		SwingWorker<Passenger, String> FirstCheckinWorker = new SwingWorker<Passenger, String>() {
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
					    ...
					    String gfirstcheckin = "";
						gfirstcheckin = gfirstcheckin + (passengersinqueue.BusinessFirstPicker().getFullName()) + "\n";
						publish(gfirstcheckin);
			...
			@Override
			protected void process(List<String> chunks) {
				String gfirstcheckin = chunks.get(chunks.size() - 1);
				desk1TxtArea.setText(gfirstcheckin);
			}
		};

		SwingWorker<Passenger, String> EconomyCheckinWorker1 = new SwingWorker<Passenger, String>() {
			...
			protected void process(List<String> chunks) {
				...
				ecoDesk1TxtArea.setText(gecocheckin);
				}
		};
			...
			protected Passenger doInBackground() throws Exception {
				while (getState() != null) {
					try {
					    ...
					    String gecocheckin = "";
						gecocheckin = gecocheckin + (passengersinqueue.EconomyPicker1().getFullName());
						//used to update the progress bars, will replace with get method for flight carrier
						updateFlightVolumes("British Airways");
						updateFlightVolumes("Qatar Airways");
						updateFlightVolumes("Emirates Airlines");
