...
public class GUIMain extends JFrame implements Observer {
	...
	private JPanel northPanel, centerPanel, southPanel, FirstandBusinessDesk, economyDesk1, economyDesk2, economyDesk3,
			processButtonPanel, buttonPanel1, buttonPanel2, buttonPanel3, buttonPanel4, checkinQueue, firstcheckinQueue,
			securityQueue, boardingQueue, checkInPanel, checkinLabelPanel, secBoardPanel, queueGroupPanel,
			emiratesDisplay, baDisplay, qatarDisplay, baProgressBarPanel, baVolumeProgresBarPanel,
			baWeightProgresBarPanel, qaProgressBarPanel, qaVolumeProgresBarPanel, qaWeightProgresBarPanel,
			eaProgressBarPanel, eaVolumeProgresBarPanel, eaWeightProgresBarPanel, baPassengerProgresBarPanel,
			qaPassengerProgresBarPanel, eaPassengerProgresBarPanel, timerPanel;
	...
	private JLabel desk1Lbl, ecoDesk1Label, ecoDesk2Label, ecoDesk3Label, checkinQueuelabel, firstcheckinQueuelabel,
			...
			qaPassengerLabel, eaPassengerLabel, timerLabel;
	...
	private JProgressBar baWeight, baVolume, qaWeight, qaVolume, eaWeight, eaVolume, baPassenger, eaPassenger,
			qaPassenger;
	...
	public GUIMain(PassengersInQueue pq, Passenger p) {
	...
	public void startDisplay() {
		SwingWorker<Void, LinkedList<Passenger>> EconomyWorker = new SwingWorker<Void, LinkedList<Passenger>>() {
			...
			protected void process(List<LinkedList<Passenger>> chunks) {
				...
				for (Passenger p : geco1) {
					count++;
					checkinQueueTxtArea.setText(new Date() + "\n" + p.getFullName() + " has joined the line"
							+ System.lineSeparator() + "CheckedIn?:" + "\t" + p.getCheckInStatus() + "\n" + geco1.size()
							+ " are now in the line" + "\n");
				}
		...
		SwingWorker<Passenger, LinkedList<Passenger>> BoardingWorker = new SwingWorker<Passenger, LinkedList<Passenger>>() {
			...
			protected void process(List<LinkedList<Passenger>> chunks) {
				...
				for (Passenger p : gboarding) {
					...
					try {
						count++;
						boardingQueueTxtArea.append(p.getFullName() + "\t" + " is now boarding flight: " + "\t"
								+ p.getflightCode() + "\t" + count + System.lineSeparator());
		...
		SwingWorker<Baggage, Baggage> EconomyCheckinWorker1 = new SwingWorker<Baggage, Baggage>() {
			...
			protected void process(List<Baggage> chunks) {
				Baggage gecocheckin = chunks.get(chunks.size() - 1);
				try {
					// System.out.println("Passssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss"+gecocheckin.getPassenger().getFullName());
		...
		SwingWorker<Passenger, Date> TimeKeeper = new SwingWorker<Passenger, Date>() {
			...
			protected void process(List<Date> chunks) {
				...
				if (currentDate.after(dateAfter30Seconds)) {
				   ...
				   if(diifLong<100000) {
					timeArea.setText("Warning!!!!!!!"+"\n"+"TIME BEFORE GATES CLOSE: " +"\t"+ Long.toString(diifLong).substring(0, 2)+"   seconds"+ "\n"+"TIME OF DEPATURE: "+"\t"+dateAfter30Seconds + "\n"
							+ "CURRENT TIME : " +"\t"+ currentDate );}
					if(diifLong<12000) {
