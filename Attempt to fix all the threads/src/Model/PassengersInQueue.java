	...
	public void puti() throws InterruptedException {
		...
		while (true) {
			...
			synchronized (lock) {
				...
				q1.add(p1);
				count++;
				System.out.println("The are currently " + count + "people in the line");
				// q2.add(p1);
		...
		synchronized (lock) {
			while (true) {
				...
				try {
					...
					for (int i = 0; i <=q1.size(); i++) {
						...
						try {
							...
							if (p.getpClass().equalsIgnoreCase("economy")) {
								...
								forSecurity.add(p);
								q1.remove();
								 System.out.println("Q1 SIZE AFTER REMOVAL FROM ECONNOMY IS:"+ q1.size());
								synchronized (Checkedin) {
									Checkedin.add(p);

								}
								System.out.println(
										"now we have this number of checkedgoats in the line " + Checkedin.size());
								System.out.println(
										"now we have this number of securitygoats in the line " + forSecurity.size());
								...
								System.out.println(
										"The check-in bag is overweight by " + b.excessBaggageWeight() + " kg.");
								System.out.println("The check-in bag exceeds volume allowance by "
										+ b.excessBaggageVolume() + " cubic inches.");
							...
							// once thecode block finishes the lock is released and other threads are
							// notified
							lock.wait();
					...
					System.out.println(
							"The state in finall block of the economy is " + Thread.currentThread().getState());
				}
			}
		...
		synchronized (lock) {
			while (true) {
				...
				try {
					...
					while (q1.size() == 0) {
						lock.wait();
						System.out.println("WAITING BUSIness  class and first clas customers.............. ");
					}
					...
					for (int i = 0; i <=q1.size(); i++) {
						...
						try {
							...
							// boolean o = !p.getpClass().equalsIgnoreCase("first")||
							...
							if (p.getpClass().equalsIgnoreCase("first") || p.getpClass().equalsIgnoreCase("business")) {
								...
								q1.remove();
                                 System.out.println("Q1 SIZE AFTER REMOVAL IS:"+ q1.size());
								...
								System.out.println("now we have this number of checkedgoats in the line from business "
										+ Checkedin.size());
								System.out.println("now we have this number of securitygoats in the line from business "
										+ forSecurity.size());
								// This should be visible on GUIMain
								System.out.println(p.getpClass() + " class passenger, " + p.getFullName()
								...
								System.out.println("The excess baggage fee is " + b.excessBaggageFee() + " GBP.");
								System.out.println(
										"Total number of passengers who have checked in is " + Checkedin.size());
								System.out.println("Passengers ready for security: " + forSecurity.size());
							...
							// System.out.println("No First or Business class XXXXXXXXXX passengers waiting
							// to be checked-in");
						}

					}
				} catch (NullPointerException | NoSuchElementException N) {
					// System.out.println("No First or Business class passengers waiting to be
					// checked-in");
				} finally {
					...
					System.out.println(
							"The state in finally block of the business is " + Thread.currentThread().getState());
				}
			}
		...
		synchronized (lock) {
			while (true) {
				...
				try {
					...
					// list
					while (forSecurity.size() == 0) {
						lock.wait();
						System.out.println("Security Queue empty.............");
					}
					...
					for (int i = 0; i <= forSecurity.size(); i++) {
						...
						if (p.getCheckInStatus() == true) {
							...
							if (forSecurity.contains(p)) {
								...
								boarding.add(p);
								System.out.println("Boarded = " + boarding.size());
							}
					...
					// .out.println("State after null pointer exception: "+
					...
					System.out.println("State in finally for security: " + Thread.currentThread().getState());
		...
		synchronized (lock) {
			while (true) {
				...
				try {
					...
					System.out.println(
							"The state in finall block of the boarding is " + Thread.currentThread().getState());
					System.out.println("The size of the boarding queue is:" + boarding.size());
				}
			}
