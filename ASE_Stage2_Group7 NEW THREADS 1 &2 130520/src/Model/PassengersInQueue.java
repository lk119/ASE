				...
				for (int i = 0; i <= q.size(); i++) {
					...
					try {
						...
						if (p.getpClass().equalsIgnoreCase("economy")) {
							...
							b.getHeight() + " X " + b.getBreadth() + " X " + b.getWidth() + " cubic inches. Therefore, the baggage "
									+ "volume is " + b.baggageVolume() + " cubic inches");
					...
					for (int i = 0; i <= q.size(); i++) {
						...
						try {
							...
							if (p.getpClass().equalsIgnoreCase("first") || p.getpClass().equalsIgnoreCase("business") ) {
								...
								System.out.println(p.getpClass() + " class passenger, " + p.getFullName()
										+ " travelling by " + p.getflightCode() + " is Checking in at Business & First Class "
										+ "Check-in Desk");
