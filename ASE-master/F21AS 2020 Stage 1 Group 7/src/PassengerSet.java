...
public class PassengerSet {
	...
	public Passenger findBooking(String bookingReferenceNum, String LastName) throws NoMatchingBookingReference, InvalidBookingReference,
	NoMatchingLastNameException {
		for (Passenger p : PassengerSet) {
			String s= p.getBookingReferenceNum();
			if (s.length() < 0 || s.length() > 8) {
				throw new InvalidBookingReference ();
        }
		else if (p.getBookingReferenceNum() == (bookingReferenceNum) 
		 ...
		 else throw new NoMatchingBookingReference (bookingReferenceNum); 
 ...
 //Treemap method to checkin status and flightcode using associated Last name
   ...
   /**
	 * TreeMap method to get passenger checkin status and flight code
	 * 
	 * @param key as booking reference number and value as check-in status and flight code
	 * @return both keys and values
	 */ 
