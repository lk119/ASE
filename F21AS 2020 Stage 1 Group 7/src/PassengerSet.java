...
//	}
	...
	public int totalNumberOfPassengersCheckedIn() {
		 ...
		 //Set<Integer> passengersCheckedIn = new HashSet<Integer>(list);  
	       // for (Integer i : passengersCheckedIn) {  
	             //System.out.println(i + ": " + Collections.frequency(list, i));  
	        //}  
		int passengersCheckedIn = 0;
		for (Passenger p : PassengerSet) {
			if (p.getCheckInStatus() != false) { passengersCheckedIn++;
			}
	...
	// as the Key
	/*
	 * public int getPassengerStatusfromLastName() { TreeMap<Boolean, String>
	 * PassengerStatus = new TreeMap<Boolean, String>(); for (Passenger p :
	 * PassengerSet) { int count = 0; PassengerStatus.put((p.getCheckInStatus()),
	 * p.getflightCode() ); Collections.frequency(PassengerStatus, "true");
	 * 
	 * } for (Entry<String, Integer> entry : PassengerStatus.entrySet()) { return
	...
	public HashMap createAMap2() {
		...
		for (Passenger p:PassengerSet ) {
			flyers.put(p.getBookingReferenceNum(),p.getCheckInStatus()+"");
		}
		return flyers;
	}
