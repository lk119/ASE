...
/**
 * Advanced Software Engineering - Coursework stage 2 Creates an set to store
 * the passenger objects inputed from the input file
 *
 * @author Lynsey Kirk, Kuda Mugara, Rachana Patel, Cameron Scott, Robert Stone
 */

public class PassengerSet implements Runnable {
	...
	public PassengerSet (PassengersInQueue q) {
					this.q = q;
		}
	
	HashMap<String, String> flyers = new HashMap<String, String>();
	// method to put random passengers from the input file into PassengersInQueue shared object
		public void run() {
			HashMap<String, String> flyers = new HashMap<String, String>();
			//PassengerSet = new HashSet<Passenger>();
			for (Passenger p : PassengerSet) {
				flyers.put(p.getBookingReferenceNum(), p.getflightCode());
			}
			
			//int size = PassengerSet.size();
