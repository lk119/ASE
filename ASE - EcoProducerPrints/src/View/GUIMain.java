...
//MVC pattern - allows listeners to be added
	...
	public void disableProcessButton() {
		...
		// Replaces EcoProducer as SwingWorker replaces the need for traditional runnables 
...
//				
//				//Thread thread1 = new Thread(new BoardingRunnable(passengersinqueue));
//				
//				  Thread thread1 = new Thread(new EcoProducer(passengersinqueue));
//				  
//				  // this allows the "process" method below to access the published variable whilst running the thread
//				  
//				  thread1.start(); 
//				  
//				  publish(passengersinqueue.getCurrentPassenger());
//				  
//				  try {				  
//					 thread1.join();
//
...
//				        		       		
				...
				try {
					...
					//String g = passengersinqueue.economyQueueputi().toString();
					passengersinqueue.economyQueueputi();
					
					//iterates through the link list of passengers in the queue in real time and adds them to a string
					Iterator<Passenger> iterator = passengersinqueue.economyQueueputi().iterator();
					String g = ""; 
					while (iterator.hasNext()) { 
					   g = g + (iterator.next().getFullName()) + "\n"; 

					}
					
					//sends passengers in the queue to the process() method below
					 publish (g); 
			...
			// this takes the chunks of string sent from the publish() method whilst the thread is running 
			@Override
			protected void process(List<String> chunks) {
				String reportB = chunks.get(chunks.size()-1);
				//if (reportB != null) {
				checkinQueueTxtArea.setText(reportB);
				//}
			}
...
//			  Thread thread2 = new Thread(new FirstProducer(passengersinqueue)); 
...
//			  
//			  thread1.start(); 
//			  publish(passengersinqueue.getQ().toString());
//			  thread2.start(); 
//			  thread3.start(); 
//			  thread4.start();
//			  thread5.start(); 
